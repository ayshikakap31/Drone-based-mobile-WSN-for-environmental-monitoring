...
for i in range(0, len(pts1)):
    pts2.append([pts1[i][0] + 24, pts1[i][1]])
...
while t < 3:
    ...
    XYTPtsT2Drones.append([pts2[i][0], pts2[i][1], t])
    i += 2
    i %= len(pts1)
    t += 1

actualGroundTruth = np.zeros((3, entries, entries))
for z in range(3):
    for i in range(48):
        for j in range(48):
...
for i in range(0, len(XYTPts)):
    twoDrones.append(groundTruth[XYTPts[i][0]][XYTPts[i][1]][XYTPts[i][2]])
...
twoDronesNearestInterpolated = np.zeros((3, entries, entries))
twoDronesIDWInterpolated = np.zeros((3, entries, entries))
  ...
  for i in range(0, entries):
    for j in range(0, entries):
      if (groundTruth[i][j][z] != 0):
        co += 1
        overall_idw_rmse += ((groundTruth[i][j][z]-twoDronesIDWInterpolated[z][i][j])/groundTruth[i][j][z]) **2
        overall_nearest_rmse += ((groundTruth[i][j][z]-twoDronesNearestInterpolated[z][i][j])/groundTruth[i][j][z]) **2
