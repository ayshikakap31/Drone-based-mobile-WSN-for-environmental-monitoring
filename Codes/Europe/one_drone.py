...
for i in range(0, len(XYTPts)):
    oneDrones.append(groundTruth[XYTPts[i][0]][XYTPts[i][1]][XYTPts[i][2]])
...
oneDronesNearestInterpolated = np.zeros((3, entries, entries))
oneDronesIDWInterpolated = np.zeros((3, entries, entries))
  ...
  for i in range(0, entries):
    for j in range(0, entries):
      if (groundTruth[i][j][z] != 0):
        co += 1
        overall_idw_rmse += ((groundTruth[i][j][z]-oneDronesIDWInterpolated[z][i][j])/groundTruth[i][j][z]) **2
        overall_nearest_rmse += ((groundTruth[i][j][z]-oneDronesNearestInterpolated[z][i][j])/groundTruth[i][j][z]) **2
